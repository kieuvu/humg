...
import 'package:info_selection/models/address_info.dart';
import 'package:info_selection/models/district.dart';
import 'package:info_selection/models/province.dart';
import 'package:info_selection/models/user_info.dart';
...
class _Step1FormState extends State<Step1Form> {
  ...
  //Hàm xác thực số điện thoại bằng biểu thức chính uy
...
class _Step2FormState extends State<Step2Form> {
  ...
  /// chuyển đổi nó thành các đối tượng tương ứng.
  ///
  /// Nếu có lỗi xảy ra trong quá trình tải dữ liệu hoặc chuyển đổi, phương
  /// thức sẽ bắt và ghi log ra màn hình debug.
  ///
  /// Bắn ra một ngoại lệ Throws [Exception]:
  ...
  Future<void> loadLocationData() async {
    try {
      ...
      // Trích xuất danh sach các xã từ dữ liệu từ điển và chuyển đổi thành
              ...
              optionsBuilder: (textEditingValue) {
                if (textEditingValue.text.isEmpty) {
                  return districtList.where((element) =>
                      ...
                      element.provinceId == widget.userInfo.address?.province?.id);
                }
                return districtList.where((element) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoItem('Tính / Thành phố:', userInfo.address?.province?.name),
...
Future<void> saveUserInfo(UserInfo info) async {
  return await Localstore.instance.collection('user').doc('info').set(info.toMap());
}
